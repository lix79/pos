...
{
	if((MY_NMEA_GNRMC_Analysis(UTC_TIME,(u8*)UTC_BUF)==0)&&(GPS_PV==1))	//GPS UTC时间信息解析   如果解析正确
	{
		return  UTC_FREE;   //返回空闲  成功
	}
  return  UTC_ERR;   //返回错误
...
// else return 1;   //时间解析错误	
   	...
   	posx=NMEA_Comma_Pos(buf,2);								//得到UTC年时间
	if(posx!=0XFF)
	{
	//	GPS_TIME->sec =(*(buf+posx-5))*10+*(buf+posx-4) ;  //得到UTC年时间	 
		GPS_TIME->sec =(*(buf+posx-6)-48)*10+(*(buf+posx-5)-48) ;
		GPS_TIME->min =(*(buf+posx-8)-48)*10+(*(buf+posx-7)-48) ;  //得到UTC年时间	
	  GPS_TIME->hour =(*(buf+posx-10)-48)*10+(*(buf+posx-9)-48) ;  //得到UTC年时间	
	}
 else return 1;   //时间解析错误	 

	posx=NMEA_Comma_Pos(buf,3);								//得到UTC年时间
	if(posx!=0XFF)
	{
		GPS_TIME->sday = (*(buf+posx-3)-48)*10+(*(buf+posx-2)-48) ;   //得到UTC年时间	 
	}
 else return 1;   //时间解析错误	

	posx=NMEA_Comma_Pos(buf,4);								//得到UTC月时间
	if(posx!=0XFF)
	{
		GPS_TIME->smon =(*(buf+posx-3)-48)*10+(*(buf+posx-2)-48) ;  	//得到UTC月时间	 
	}
  else return 1;   //时间解析错误	

	posx=NMEA_Comma_Pos(buf,5);								//得到UTC 日时间
	if(posx!=0XFF)
	{
		GPS_TIME->syear=(*(buf+posx-5)-48)*1000+(*(buf+posx-4)-48)*100+(*(buf+posx-3)-48)*10+(*(buf+posx-2)-48) ; //得到UTC 日时间	 
	}
 else return 1;   //时间解析错误		
	
  GPS_TIME_AMEND(GPS_TIME,0);      //时间数据修正  UTC数据修正成GPS时间  并修正凌晨0点至8点日期错误
...
{
	if((MY_NMEA_TIMEA_Analysis(GPS_TIME,(u8*)GPS_BUF)==0)&&(GPS_PV==1))	//GPS UTC时间信息解析   如果解析正确
	{
		return  GPS_FREE;   //返回空闲  成功
	}
	...
	if((GPS_BUF->GPS_STA&GPS_BUSY)==0)   //如果GPS缓存正在解析
	{
     ...
     if(GPS_BUF->GPS_STA&UTC_OVER)	//如果已经收到0X0D  后面再收到0X0A 则表示结束
       ...
       if(RX_BUF==0X0D)  //收到第一个结束标记
		...
		else if(RX_BUF =='G')  //如果收到G字符  可能是帧头
		{
	    GPS_BUF->GPS_NUM = 0;      //写指针清零   状态及缓存不处理
			GPS_W_BUF_Byte(GPS_BUF,RX_BUF);  //写入缓存	缓存的第1个元素 为 G
		}
		else if(GPS_BUF_Valid_NUM(GPS_BUF)>0)  //已经接收到G字符
		{
			if(((RX_BUF =='P')||(RX_BUF =='N'))&&(GPS_BUF_Valid_NUM(GPS_BUF)==1))  //缓存的第2个元素 为 P
		  {
		    GPS_W_BUF_Byte(GPS_BUF,RX_BUF);  //写入缓存	
		  }
		  else if((RX_BUF =='Z')&&(GPS_BUF_Valid_NUM(GPS_BUF)==2)) //缓存的第3个元素 为 Z
		  {
		    GPS_W_BUF_Byte(GPS_BUF,RX_BUF);  //写入缓存	
		  }
		  else if((RX_BUF =='D')&&(GPS_BUF_Valid_NUM(GPS_BUF)==3)) //缓存的第4个元素 为 D
		  {
		    GPS_W_BUF_Byte(GPS_BUF,RX_BUF);  //写入缓存
		  }
