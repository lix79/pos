...
#define CAMM_IO_T_Nuber    3     //扫描周期  单位 ms

u8  CAMM_IO_TIME_Nuber = 0;      //快门引脚判别状态扫描变量   最高位快门扫描间隙标记
_calendar_obj Cam_Save_time;	   //航拍时间记录变量

/**************************实现函数********************************************
*函数原型:	   	void EXTI1_IRQHandler(void)
*功　　能:	   	拍照按键外部中断服务函数
*输入参数：  		无
*输出参数：  		无
*******************************************************************************/
...
void TIM4_IRQHandler(void)   // 1 MS中断一次
{
 ...
 //闪光灯指示任务 -------------------------------------- 
    ...
    #endif
//---------------------------------------------------
	 if(CAMM_IO_TIME_Nuber&0x80)  //如果已经触发了
	 {
		if(KMM == 1)  //如果触发结束了
		{
		  CAMM_IO_TIME_Nuber = 0 ;       //可以开始下一次快门触发
		}
	 }
	 else if( (CAMM_IO_TIME_Nuber&0x7f) > 0)  //如果有需要扫描 且为有效电平
	 {
		if(KMM == 0)
		{
	   ...
	   if(CAMM_IO_TIME_Nuber == 0)  //如果扫描完了   则认为为有效数据  写入FIFO
	   {
		   if(Cam_FIFO_WRITE(TIME_FIFO,&Cam_Save_time)==Cam_FIFO_OK)   //时间写入 FIFO  
	     {
		       ...
		       SET_Cal_LED;   //设置快门指示灯任务标记
			     ...
			     LED0_ON;       //点亮快门指示灯 	
         #endif
			     CAMM_IO_TIME_Nuber = 0x80 ;   //装载不响应时间间隙
	     }	
       else
			 {
			 }				 
