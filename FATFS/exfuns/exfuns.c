...
FIL *ftemp_gps;	  		//文件3.     gps航拍时间记录文件
FIL *ftemp_pos;	  		//文件3.     gps航拍时间记录文件
UINT br,bw;			//读写变量
...
u8 exfuns_init(void)
{
	...
	ftemp_gps=(FIL*)mymalloc(sizeof(FIL));		//为ftemp申请内存
	ftemp_pos=(FIL*)mymalloc(sizeof(FIL));		//为ftemp申请内存
	fatbuf=(u8*)mymalloc(512);				  //为fatbuf申请内存
	if(i==_VOLUMES&&file&&ftemp&&ftemp_gps&&fatbuf)return 0;  //申请有一个失败,即失败.
	else return 1;	
}
